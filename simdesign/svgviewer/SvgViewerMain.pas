...
begin
  ...
  // so we can just create it with Self, and do not have to destroy it.
    ...
    try
      ...
      begin
        ...
        try
          ...
          try
            RawStream := TStringStream.Create('');
            RawStream.CopyFrom(FS, FS.Size);
            RawStream.Position := 0;
            FS.Position := 0;
            mmSource.Lines.Add(RawStream.DataString);
          finally
            RawStream.Free;
          end;
...
begin
  ...
  begin
    ...
    begin
      ...
      ShowMessage(Format('you clicked %s!', [TpgText(Parent.Elements[1]).Text.Value]));
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    // precision
    Line.X1.Value := random(W);
    Line.Y1.Value := random(H);
    Line.X2.Value := random(W);
    Line.Y2.Value := random(H);
    ...
    case i mod 4 of
    0: Line.StrokeWidth.Value := 0.7;
    1: Line.StrokeWidth.Value := 1.3;
    2: Line.StrokeWidth.Value := 2.5;
    3: Line.StrokeWidth.Value := 3.6;
    end;
...
begin
  ...
  for i := 1 to 100 do
  begin
    Rectangle := TpgRectangle.Create(Parent);

    // Set width and height
    Rectangle.Width.Value := random(W div 3 + 20);
    Rectangle.Height.Value := random(H div 3 + 20);
    ...
    if i mod 4 = 0 then
    begin
      R := pgMin(Rectangle.Width.Value / 4, Rectangle.Height.Value / 4);
      Rectangle.RX.Value := R;
      Rectangle.RY.Value := R;
    end;
    ...
    Rectangle.FillOpacity.Value := 0.5;

    // Set strokewidth
    Rectangle.StrokeWidth.Value := 2;

    // Do some rotation around rectangle center, then translate it
    ATransform := TpgAffineTransform.Create;
    ATransform.Rotate(Random(40) - 20, Rectangle.Width.Value/2, Rectangle.Height.Value/2);
    ATransform.Translate(random(W), random(H));
    Rectangle.Transform.Value := ATransform;
...
begin
  ...
  // Create a path shape
  PathShape := TpgPathShape.Create(Parent);
  PathShape.Path.Value := ConstructPath;
  PathShape.Stroke.AsColor32 := clGreen32;
  PathShape.StrokeWidth.Value := 4.0;
  ...
  PathShape.FillOpacity.Value := 0.5;
  ...
  PathShape.Transform.Value := ATransform;
...
begin
  ...
  for i := 1 to 100 do
  begin
    Ellipse := TpgEllipse.Create(Parent);

    // Set X and Y lengths of the radii
    Ellipse.Rx.Value := random(W div 6 + 10);
    Ellipse.Ry.Value := random(H div 6 + 10);
    ...
    // This sets the fill to semi-transparent (0=transparent, 1=opaque)
    Ellipse.FillOpacity.Value := 0.5;

    // Set strokewidth
    Ellipse.StrokeWidth.Value := 2;

    // Do some rotation around ellipse center, then translate it
    ATransform := TpgAffineTransform.Create;
    ATransform.Rotate(Random(40) - 20, 0, 0);
    ATransform.Translate(random(W), random(H));
    Ellipse.Transform.Value := ATransform;
...
begin
  ...
  for i := 0 to 17 do
  begin
    Text := TpgText.Create(Parent);

    // Font and size
    Text.FontFamily.Value := 'Verdana';
    Text.FontSize.Value := 20;
    ...
    case i mod 4 of
//    0: Span.FontWeight.Value := fwBold;
    1: Text.Fill.AsColor32 := clRed32;
    2:
      begin
        Text.Fill.AsColor32 := clLime32;
        Text.Stroke.AsColor32 := clGreen32;
        Text.StrokeWidth.Value := 0.05; // check this!}
      end;
//    3: Span.FontStyle.Value := fsItalic;
    end;//case
    ...
    Text.Transform.Value := ATransform;
...
begin
  ...
  begin
    ...
    begin
      Rectangle := TpgRectangle.Create(Parent);
      // Set width and height
      Rectangle.Width.Value := RW;
      Rectangle.Height.Value := RH;
      // set X and Y
      Rectangle.X.Value := i * RW;
      Rectangle.Y.Value := j * RH;
      // Color
      Rectangle.Fill.AsColor32 := RandomColor;
    end;
  end;
...
begin
  ...
  begin
    ...
    // update the loop
    Ang := Ang - 0.5;
    FPS := 1000 / max(pgGetTickCount - DT, 1);
    DT := pgGetTickCount;
...
procedure TfrmMain.mnuClickmeClick(Sender: TObject);
  ...
  var
    ...
    Text: TpgText;
  begin
    Group := TpgGroup.Create(FScene.ViewPort);
    Rect := TpgRectangle.Create(Group);
    Rect.X.Value := X;
    Rect.Y.Value := Y;
    Rect.Width.Value := W;
    Rect.Height.Value := H;
    ...
    Rect.StrokeWidth.Value := 2.5;
    Rect.Fill.AsColor32 := clTeal32;
    Text := TpgText.Create(Group);
    Text.Text.Value := AText;
...
begin
  ...
  // the viewport.
  FScene.ViewPort.Width.Value := W;
  FScene.ViewPort.Height.Value := H;
  // 4x3 buttons
  BX := W / 4;
  BY := H / 3;
  BW := BX * 0.8;
  BH := BY * 0.6;
  // font
  FScene.ViewPort.FontFamily.Value := 'Verdana';
