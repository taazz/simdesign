{ unit NativeXml
  ...
  Copyright (c) 2003-2008 Simdesign B.V.
...
type
  ...
  TXmlFormatType = (
    ...
    xfCompact     // Save without any control AnsiChars except LF after declarations
  );
  ...
  TXmlElementType = (
    ...
    xeExclam,      // Any <!data>
    xeQuestion,    // Any <?data?>
    xeCharData,    // AnsiCharacter data in a node
    xeUnknown      // Any <data>
  );
  ...
  TBinaryEncodingType = (
    ...
    xbeBase64   { With this encoding, each group of 3 bytes are stored as 4
                  AnsiCharacters, requiring 64 different AnsiCharacters.}
  );
...
type
  ...
  protected
    ...
    procedure ReadFromString(const AValue: RawByteString); virtual;
  ...
  public
    ...
    procedure AttributeAdd(const AName, AValue: UTF8String); {$IFDEF D4UP}overload;{$ENDIF}
    ...
    // be returned.
    function ReadAttributeBool(const AName: UTF8String; ADefault: boolean {$IFDEF D4UP}= False{$ENDIF}): boolean; virtual;
    function ReadAttributeDateTime(const AName: UTF8String; ADefault: TDateTime {$IFDEF D4UP}= 0{$ENDIF}): TDateTime; virtual;
    // Find the attribute with AName, and convert its value to an integer. If the
    // attribute is not found, or cannot be converted, the default ADefault will
    // be returned.
    function ReadAttributeInteger(const AName: UTF8String; ADefault: integer {$IFDEF D4UP}= 0{$ENDIF}): integer; virtual;
    // Find the attribute with AName, and convert its value to an int64. If the
    // attribute is not found, or cannot be converted, the default ADefault will
    // be returned.
    function ReadAttributeInt64(const AName: UTF8String; ADefault: int64 {$IFDEF D4UP}= 0{$ENDIF}): int64; virtual;
    // Find the attribute with AName, and convert its value to a float. If the
    // attribute is not found, or cannot be converted, the default ADefault will
    // be returned.
    function ReadAttributeFloat(const AName: UTF8String; ADefault: double {$IFDEF D4UP}= 0{$ENDIF}): double;
    function ReadAttributeString(const AName: UTF8String; const ADefault: UTF8String {$IFDEF D4UP}= ''{$ENDIF}): UTF8String; virtual;
    ...
    // be returned.
    function ReadBool(const AName: UTF8String; ADefault: boolean {$IFDEF D4UP}= False{$ENDIF}): boolean; virtual;
    {$IFDEF USEGRAPHICS}
    ...
    function ReadColor(const AName: UTF8String; ADefault: TColor {$IFDEF D4UP}= clBlack{$ENDIF}): TColor; virtual;
    ...
    // subnode is not found, or cannot be converted, ADefault will be returned.
    function ReadDateTime(const AName: UTF8String; ADefault: TDateTime {$IFDEF D4UP}= 0{$ENDIF}): TDateTime; virtual;
    // Read the subnode with AName and convert its value to a double. If the
    // subnode is not found, or cannot be converted, ADefault will be returned.
    function ReadFloat(const AName: UTF8String; ADefault: double {$IFDEF D4UP}= 0.0{$ENDIF}): double; virtual;
    // Read the subnode with AName and convert its value to an int64. If the
    // subnode is not found, or cannot be converted, ADefault will be returned.
    function ReadInt64(const AName: UTF8String; ADefault: int64 {$IFDEF D4UP}= 0{$ENDIF}): int64; virtual;
    // Read the subnode with AName and convert its value to an integer. If the
    // subnode is not found, or cannot be converted, ADefault will be returned.
    function ReadInteger(const AName: UTF8String; ADefault: integer {$IFDEF D4UP}= 0{$ENDIF}): integer; virtual;
    // Read the subnode with AName and return its UTF8String value. If the subnode is
    // not found, ADefault will be returned.
    function ReadString(const AName: UTF8String; const ADefault: UTF8String {$IFDEF D4UP}= ''{$ENDIF}): UTF8String; virtual;
    // Read the subnode with AName and return its UnicodeString value. If the subnode is
    // not found, ADefault will be returned.
    function ReadUnicodeString(const AName: UTF8String; const ADefault: UnicodeString {$IFDEF D4UP}= ''{$ENDIF}): UnicodeString; virtual;
    ...
    procedure WriteAttributeBool(const AName: UTF8String; AValue: boolean; ADefault: boolean {$IFDEF D4UP}= False{$ENDIF}); virtual;
    procedure WriteAttributeDateTime(const AName: UTF8string; AValue: TDateTime; ADefault: TDateTime {$IFDEF D4UP}= 0{$ENDIF}); virtual;
    ...
    // WriteOnDefault = False, no attribute will be added.
    procedure WriteAttributeInteger(const AName: UTF8String; AValue: integer; ADefault: integer {$IFDEF D4UP}= 0{$ENDIF}); virtual;
    procedure WriteAttributeInt64(const AName: UTF8String; const AValue: int64; ADefault: int64 {$IFDEF D4UP}= 0{$ENDIF}); virtual;
    procedure WriteAttributeFloat(const AName: UTF8String; AValue: double; ADefault: double {$IFDEF D4UP} = 0{$ENDIF}); virtual;
    ...
    // will be added.
    procedure WriteAttributeString(const AName: UTF8String; const AValue: UTF8String; const ADefault: UTF8String {$IFDEF D4UP}= ''{$ENDIF}); virtual;
    ...
    procedure WriteBool(const AName: UTF8String; AValue: boolean; ADefault: boolean {$IFDEF D4UP}= False{$ENDIF}); virtual;
    {$IFDEF USEGRAPHICS}
    ...
    // AValue. If AValue = ADefault, and WriteOnDefault = False, no subnode will be added.
    procedure WriteColor(const AName: UTF8String; AValue: TColor; ADefault: TColor {$IFDEF D4UP}= clBlack{$ENDIF}); virtual;
    ...
    procedure WriteDateTime(const AName: UTF8String; AValue: TDateTime; ADefault: TDateTime {$IFDEF D4UP}= 0{$ENDIF}); virtual;
    // Add or replace the subnode with AName and set its value to represent the double
    // AValue. If AValue = ADefault, and WriteOnDefault = False, no subnode will be added.
    procedure WriteFloat(const AName: UTF8String; AValue: double; ADefault: double {$IFDEF D4UP}= 0.0{$ENDIF}); virtual;
    // Add or replace the subnode with AName and set its value to represent the hexadecimal representation of
    // AValue. If AValue = ADefault, and WriteOnDefault = False, no subnode will be added.
    procedure WriteHex(const AName: UTF8String; AValue: integer; Digits: integer; ADefault: integer {$IFDEF D4UP}= 0{$ENDIF}); virtual;
    // Add or replace the subnode with AName and set its value to represent the int64
    // AValue. If AValue = ADefault, and WriteOnDefault = False, no subnode will be added.
    procedure WriteInt64(const AName: UTF8String; AValue: int64; ADefault: int64 {$IFDEF D4UP}= 0{$ENDIF}); virtual;
    // Add or replace the subnode with AName and set its value to represent the integer
    // AValue. If AValue = ADefault, and WriteOnDefault = False, no subnode will be added.
    procedure WriteInteger(const AName: UTF8String; AValue: integer; ADefault: integer {$IFDEF D4UP}= 0{$ENDIF}); virtual;
    // Add or replace the subnode with AName and set its value to represent the UTF8String
    // AValue. If AValue = ADefault, and WriteOnDefault = False, no subnode will be added.
    procedure WriteString(const AName, AValue: UTF8String; const ADefault: UTF8String {$IFDEF D4UP}= ''{$ENDIF}); virtual;
    ...
    // AValue. If AValue = ADefault, and WriteOnDefault = False, no subnode will be added.
    procedure WriteUnicodeString(const AName: UTF8String; const AValue: UnicodeString; const ADefault: UnicodeString {$IFDEF D4UP}= ''{$ENDIF}); virtual;
  ...
  public
    ...
    // Call procedure ReadFromString to load an XML document from the RawByteString AValue.
    ...
    // Call WriteToString to save the XML document to a RawByteString. Set XmlFormat to
    // xfReadable if you want the RawByteString to contain indentations to make the XML
    // more human-readable. This is not the default and also not compliant with
    // the XML specification.
    function WriteToString: RawByteString; virtual;
...
{$ENDIF}
  ...
  private
    FBuffer: RawByteString;         // Buffer that holds temporary utf8 characters
...
function EscapeString(const AValue: UTF8String): UTF8String;

// Replace all escaped characters in string AValue by their original. This includes
// character references using &#...; and &#x...;
function UnEscapeStringUTF8(const AValue: UTF8String): UTF8String;

// Enclose the string AValue in quotes.
function QuoteString(const AValue: UTF8String): UTF8String;

// Remove the quotes from string AValue.
function UnQuoteString(const AValue: UTF8String): UTF8String;

// This function adds control characters Chars repeatedly after each Interval
// of characters to RawByteString Value.
function AddControlChars(const AValue: RawByteString; const Chars: RawByteString; Interval: integer): RawByteString;

// This function removes control characters from RawByteString AValue (Tab, CR, LF and Space)
function RemoveControlChars(const AValue: RawByteString): RawByteString;
...
// Convert a UnicodeString to a UTF8 encoded AnsiString
...
// Convert a UTF8 encoded string to a AnsiString
function sdUtf8ToAnsi(const S: UTF8String): AnsiString;


// parse functions

// Find SubString within UTF8String S, only process AnsiCharacters between Start and Close.
// First occurrance is reported in APos. If something is found, function returns True.
function FindString(const SubString: UTF8String; const S: UTF8String; Start, Close: integer; var APos: integer): boolean;
...
function MatchString(const SubString: UTF8String; const S: UTF8String; Start: integer): boolean;
...
procedure ParseAttributes(const AValue: UTF8String; Start, Close: integer; Attributes: TStrings);
...
// If the resulting AnsiString still has a length > 0, the function returns True.
function TrimPos(const AValue: UTF8String; var Start, Close: integer): boolean;

// Encoding/Decoding functions

// Encode binary data in Source as BASE64. The function returns the BASE64 encoded
// data as RawByteString, without any linebreaks.
...
// data as RawByteString. Use a TStringStream to convert this data to a stream. The Source
// RawByteString may contain linebreaks and control characters, these will be stripped.
...
// data as RawByteString, without any linebreaks.
function EncodeBinHex(const Source: RawByteString): RawByteString;
...
{$ENDIF}
  ...
  sxeIllegalCharInNodeName       = 'Illegal AnsiCharacter in node name "%s"';
  ...
  sxeUnsupportedEncoding         = 'Unsupported AnsiString encoding';
  ...
  sxeCannotReadMultipeChar       = 'Cannot read multiple AnsiChars from conversion stream at once';
...
const
  ...
  // Count of different escape AnsiCharacters
  cEscapeCount = 5;

  // These are AnsiCharacters that must be escaped. Note that "&" is first since
  // when another would be replaced first (eg ">" by "&lt;") this could
  // cause the new "&" in "&lt;" to be replaced by "&amp;";
  cEscapes: array[0..cEscapeCount - 1] of AnsiString =
    ('&', '<', '>', '''', '"');

  // These are the AnsiStrings that replace the escape AnsiStrings - in the same order
  cReplaces: array[0..cEscapeCount - 1] of AnsiString =
    ('&amp;', '&lt;', '&gt;', '&apos;', '&quot;');
  ...
  // byte order marks for AnsiStrings
  // Unicode text files should contain $FFFE as first AnsiCharacter to identify such a file clearly. Depending on the system
...
constructor TsdStringStream.Create(const S: RawByteString);
begin
  inherited Create;
  SetSize(length(S));
  if Size > 0 then
...
function TsdStringStream.DataString: RawByteString;
begin
...
// Remove control AnsiCharacters from AnsiString in AValue
var
  i, j: integer;
begin
...
function MatchString(const SubString: UTF8String; const S: UTF8String; Start: integer): boolean;
// Check if the Substring matches the AnsiString S at position Start.
// Note: this funtion is case-insensitive
var
  CharIndex: integer;
begin
...
procedure ParseAttributes(const AValue: UTF8String; Start, Close: integer; Attributes: TStrings);
// Convert the attributes AnsiString AValue in [Start, Close - 1] to the attributes AnsiStringlist
var
  i: integer;
  InQuotes: boolean;
  AQuoteChar: AnsiChar;
...
begin
  ...
  until Found = False;
  // The surplus AnsiString that we already read (everything after the tag)
...
function EncodeBase64Buf(const Buffer: TBytes; Count: Integer): RawByteString;
begin
...
procedure DecodeBase64Buf(const Source: RawByteString; var Buffer: TBytes; Count: Integer);
var
  ADecoded: TBytes;
begin
...
function EncodeBinHexBuf(const Source; Count: Integer): RawByteString;
// Encode binary data in Source as BINHEX. The function returns the BINHEX encoded
// data as RawByteString, without any linebreaks.
var
{$IFDEF CLR}
...
procedure DecodeBinHexBuf(const Source: RawByteString; var Buffer{$IFDEF CLR}: TBytes{$ENDIF}; Count: Integer);
...
const
  cBoolValues: array[boolean] of AnsiString = ('false', 'true');
begin
...
// Get the binary contents of this node as Base64 and return it as a AnsiString
var
...
var
  S: TsdStringStream;
begin
  // We will simply call WriteToStream and collect the result as AnsiString using
  // a AnsiString stream
...
begin
  WriteString(AName, FromUnicodeString(AValue), ADefault);
end;
...
procedure TNativeXml.LoadFromFile(const FileName: AnsiString);
var
  S: TStream;
begin
...
procedure TNativeXml.ReadFromString(const AValue: AnsiString);
var
  S: TStream;
begin
  S := TsdStringStream.Create(AValue);
...
procedure TNativeXml.SaveToFile(const FileName: AnsiString);
var
  S: TStream;
begin
...
function TNativeXml.WriteToString: AnsiString;
var
  S: TsdStringStream;
begin
  S := TsdStringStream.Create('');
      ...
      begin
        ...
        begin
          ...
          begin
            B2 := 0;
            FStream.Read(B2, 1);
            FBuffer := FBuffer + AnsiChar(B2);
          end;
          B3 := 0;
          FStream.Read(B3, 1);
          FBuffer := FBuffer + AnsiChar(B3);
        end;
      end;
