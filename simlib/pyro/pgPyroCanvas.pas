...
uses
  ...
  pgCanvas, pgPath, pgSurface, pgTransform, pgRasterize, pgCover, pgColor, pgBitmap,
  pgStroke, pgRenderPath, pgBoundsPath, pgDocument, pgImage,
  pgSampler, pgRenderFont, pgBlend, pgRegion, pgWinGDI, Pyro;
...
var
  glLastTrialTick: dword = 0;
  glIsShowingMsg: boolean = False;

procedure ShowTrialMessage;
begin
  if glIsShowingMsg then
    exit;
  glIsShowingMsg := True;
  try
    ShowMessage(
      'Pyrographics Engine trial version'#13 +
    ...
    glLastTrialTick := GetTickCount;
  finally
    glIsShowingMsg := False;
  end;
end;
{$endif PYROTRIAL}
...
begin
  ...
  FRasterizer.RasterizePolyPolygon(APath.AsPolyPolygon, Transforms);
...
begin
  {$IFDEF PYROTRIAL}
  if GetTickCount - glLastTrialTick > 30000 then
    ShowTrialMessage;
  {$ENDIF}
...
{  BlackBoxPath := NewPath;
  ...
  BlackBoxStroke.AsColor32 := clRed32;
...
begin
  ...
  FRasterizer.Cover := FCover;
  FRasterizer.RasterizePolyPolygon(FStrokePath.AsPolyPolygon, Transforms);
