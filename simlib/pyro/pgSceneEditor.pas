...
uses
  Controls, Contnrs, Classes, SysUtils, Messages, pgSceneViewer, pgContentProvider,
  pgRender, pgPlatform, Pyro, pgViewer, pgScene, pgSyncList, pgDocument,
  pgCanvas, pgTransform, pgCoreRender, pgSelector, pgBitmap, pgPyroCanvas,
  pgWinGDI;
...
type
  ...
  TpgItemEvent = procedure (Sender: TObject; AId: longword) of object;
  ...
  private
    ...
    procedure SetHoverId(const Value: longword);
  ...
  protected
    ...
    procedure InvalidateGraphic(AId: longword);
    // Select the graphic with AId
    procedure SelectGraphic(AId: longword);
    ...
    // Id of the element that's hovered over
    property HoverId: longword read FHoverId write SetHoverId;
...
begin
  ...
  FSyncList := EditRenderer.SyncList;
...
begin
  ...
  begin
    ...
    if assigned(HitInfo.Graphic) then
      SelectGraphic(HitInfo.Graphic.Id);
...
procedure TpgSceneEditor.InvalidateGraphic(AId: longword);
var
  Item: TpgBoundsItem;
  Graphic: TpgGraphic;
  TL: TpgTransformList;
...
begin
  ...
  Graphic := TpgGraphic(Scene.ElementById(AId));
...
begin
  ...
  begin
    ...
    // Render Hover
    if FHoverHighlighting and (HoverId > 0) then
    begin
      Element := Scene.ElementById(HoverId);
...
procedure TpgSceneEditor.SceneBeforeChange(Sender: TObject; AElementId,
  APropId: longword; AChange: TpgChangeType);
begin
  inherited;
  // Here we react to changes in the scene. We must reflect changes in the
  // selection.
  FSelectors.SceneChange(Sender, AElementId, APropId, AChange);
end;

procedure TpgSceneEditor.SelectGraphic(AId: longword);
var
  Selector: TpgSelector;
begin
  InvalidateGraphic(AId);
  Selector := TpgSelector.Create(Self, Associate.Canvas, AId, Transform.PixelScale);
...
procedure TpgSceneEditor.SetHoverId(const Value: longword);
begin
  if FHoverId <> Value then
  begin
    InvalidateGraphic(FHoverId);
    FHoverId := Value;
    UpdateWindowsCursor;
    InvalidateGraphic(FHoverId);
    if assigned(FOnHoverItem) then
      FOnHoverItem(Self, FHoverId);
  end;
end;
...
begin
  ...
  if assigned(FSyncList) then
    FSyncList.Scene := Value;
