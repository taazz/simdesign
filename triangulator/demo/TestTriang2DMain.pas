...
begin
  FGraph.Segments.Add(TsdSegment2D.CreateWithVertices(V1, V2))
end;
...
begin
  ...
  begin
    ...
    if assigned(FPrevVertex) then
      FGraph.Segments.Add(TsdSegment2D.CreateWithVertices(FPrevVertex, Vertex));
...
begin
  ...
  begin
    for i := 0 to FGraph.Segments.Count - 1 do
    begin
      P1 := FGraph.Segments[i].Vertex1.Point^;
      P2 := FGraph.Segments[i].Vertex2.Point^;
  ...
  begin
    ...
    TP[0] := Triangle.Vertices[0].Point^;
    TP[1] := Triangle.Vertices[1].Point^;
    TP[2] := Triangle.Vertices[2].Point^;
    for j := 0 to 2 do begin
      if assigned(Triangle.Neighbours[j]) then
        pbMain.Canvas.Pen.Color := clBlack
      else
        pbMain.Canvas.Pen.Color := clLime;
      if assigned(Triangle.Segments[j]) and chbSegments.Checked then
    ...
    // Delaunay circle
    if chbDelaunayCircle.Checked and (Triangle is TsdDelaunayTriangle2D) then
    begin
      C := TsdDelaunayTriangle2D(Triangle).CircleCenter;
      R := sqrt(TsdDelaunayTriangle2D(Triangle).SquaredRadius);
...
begin
  ...
  try
    ...
    if FMesh.Segments.Count = 0 then
      FMesh.ConvexHull;
...
begin
  ...
  begin
    FGraph.Segments.Add(TsdSegment2D.CreateWithVertices(FPrevVertex, FFirstVertex));
    FFirstVertex := nil;
    FPrevVertex := nil;
    mnuClosePolygon.Enabled := False;
  end;
