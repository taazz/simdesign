...
type
  ...
  public
    ...
    function IntersectWith(ASegment: TsdSegment2D): TsdVertex2D;
  ...
  TsdSegment2DClass = class of TsdSegment2D;
  ...
  protected
    ...
    function GetSegments(Index: integer): TsdSegment2D; virtual;
    procedure SetSegments(Index: integer; const Value: TsdSegment2D); virtual;
  ...
  public
    ...
    // or -1 if not
    function SegmentIndex(ASegment: TsdSegment2D): integer;
    ...
    // added in a descendant class, so in the base class TsdTriangle2D nil is returned
    property Segments[Index: integer]: TsdSegment2D read GetSegments write SetSegments;
  ...
  // A mesh consisting of triangles and vertices, where each triangle contains
  ...
  protected
    ...
    function NewSegment: TsdSegment2D;
    class function GetSegmentClass: TsdSegment2DClass; virtual;
...
procedure TsdSegment2D.Assign(Source: TPersistent);
begin
  if Source is TsdSegment2D then
  begin
    FVertex1 := TsdSegment2D(Source).FVertex1;
    FVertex2 := TsdSegment2D(Source).FVertex2;
  end else
    inherited;
end;

procedure TsdSegment2D.CalculateMetrics;
var
  R: double;
begin
...
constructor TsdSegment2D.CreateWithVertices(AVertex1, AVertex2: TsdVertex2D);
begin
  Create;
  FVertex1 := AVertex1;
  FVertex2 := AVertex2;
...
function TsdSegment2D.GetCenter: TsdPoint2D;
begin
...
function TsdSegment2D.GetNormal: TsdPoint2D;
begin
  if not FValidMetrics then
    CalculateMetrics;
  Result := FNormal;
...
function TsdSegment2D.GetSquaredEncroachRadius: double;
begin
  if not FValidMetrics then
    CalculateMetrics;
...
function TsdSegment2D.IntersectWith(ASegment: TsdSegment2D): TsdVertex2D;
var
  R: TsdPoint2D;
  PosP, PosQ: double;
begin
...
procedure TsdSegment2D.Invalidate;
begin
  FValidMetrics := False;
end;

function TsdSegment2D.IsVertexOnSegment(AVertex: TsdVertex2D; APrecisionSqr: double): boolean;
begin
  Result := PointToLineDist2DSqr(AVertex.FPoint,
    FVertex1.FPoint, FVertex2.FPoint) <= APrecisionSqr;
end;

function TsdSegment2D.PointEncroaches(const P: TsdPoint2D): boolean;
var
  C: TsdPoint2D;
begin
  C := GetCenter;
...
procedure TsdSegment2D.ReplaceVertex(OldVertex, NewVertex: TsdVertex2D);
begin
...
procedure TsdSegment2D.SetVertex1(const Value: TsdVertex2D);
begin
  FVertex1 := Value;
  FValidMetrics := False;
end;

procedure TsdSegment2D.SetVertex2(const Value: TsdVertex2D);
begin
  FVertex2 := Value;
  FValidMetrics := False;
end;
...
function TsdTriangle2D.GetSegments(Index: integer): TsdSegment2D;
begin
  Result := nil;
end;
...
procedure TsdTriangle2D.InvalidateSegments;
var
  i: integer;
  S: TsdSegment2D;
begin
...
function TsdTriangle2D.SegmentIndex(ASegment: TsdSegment2D): integer;
var
  i: integer;
begin
  for i := 0 to 2 do
...
procedure TsdTriangle2D.SetSegments(Index: integer; const Value: TsdSegment2D);
begin
...
class function TsdTriMesh2D.GetSegmentClass: TsdSegment2DClass;
begin
  Result := TsdSegment2D;
end;
...
function TsdTriMesh2D.NewSegment: TsdSegment2D;
begin
