...
uses
  Dialogs, Windows, Classes, Contnrs, SysUtils, sdTriMesh2D, sdPoints2D;
...
type
  ...
  public
    ...
    property Segments: TsdSegment2DList read FSegments;
  end;

  // A Segment Triangle contains references for each edge to a segment, or nil
  // if there is no graph segment for this edge.
  TsdSegmentTriangle2D = class(TsdTriangle2D)
  private
    FSegments: array[0..2] of TsdSegment2D;
  protected
    function GetSegments(Index: integer): TsdSegment2D; override;
    procedure SetSegments(Index: integer; const Value: TsdSegment2D); override;
  end;
  ...
  private
    ...
    FSegmentChain: TsdTriangleChain2D;
  ...
  protected
    ...
    procedure ReplaceVertexInSegments(OldVertex, NewVertex: TsdVertex2D);
    ...
    function AddSegmentToTriangulation(ASegment: TsdSegment2D): boolean;
    ...
    procedure ReduceSegmentChain(AChain: TsdTriangleChain2D; ARemovals: TsdTriangle2DList); virtual;
...
begin
  ...
  FSegments := TsdSegment2DList.Create;
end;
...
begin
  ...
  for i := 0 to Count - 1 do begin
    ...
    V := TsdVertex2D.CreateWithCoords(StrToFloatDef(A, 0), StrToFloatDef(B, 0));
    Vertices.Add(V);
  end;
  Count := StrToIntDef(SL[Idx], 0); inc(Idx);
  for i := 0 to Count - 1 do begin
    Split(SL[Idx], A, B); inc(Idx);
     S := TsdSegment2D.CreateWithVertices(
...
begin
  ...
  SL.Add(IntToStr(Segments.Count));
...
{ TsdSegmentTriangle2D }

function TsdSegmentTriangle2D.GetSegments(Index: integer): TsdSegment2D;
begin
  Result := FSegments[Index mod 3];
end;

procedure TsdSegmentTriangle2D.SetSegments(Index: integer; const Value: TsdSegment2D);
var
  Idx: integer;
begin
  Idx := Index mod 3;
  if FSegments[Idx] <> Value then begin
    FSegments[Idx] := Value;
...
begin
  ...
  // Add vertices
  for i := 0 to AGraph.Vertices.Count - 1 do begin
    V := NewVertex;
  ...
  // Add segments
  for i := 0 to AGraph.Segments.Count - 1 do begin
    S := NewSegment;
...
function TsdTriangulationMesh2D.AddSegmentToTriangulation(ASegment: TsdSegment2D): boolean;
var
  i, j, E1, E2: integer;
  Triangle, T1, T2: TsdTriangle2D;
  CrossSegment: TsdSegment2D;
...
begin
  ...
  for i := 0 to FSegmentChain.Count - 2 do begin
    ...
    if assigned(CrossSegment) then begin
  ...
  // this segment
  if FSegmentChain.Count = 1 then begin
...
begin
  Result := httNone;
  ATriangle := nil;
  for i := 0 to Triangles.Count - 1 do begin
    Result := Triangles[i].HitTest(APoint);
    if not (Result in [httNone, httClose0, httClose1, httClose2]) then begin
...
begin
  ...
  for i := 0 to 2 do begin
    ...
    if N.VertexIndex(AVertex) = -1 then continue;
    // Obviously.. check if we do not have it already
    if AList.IndexOf(N) >= 0 then continue;
    // Ok, new one.. add it
    AList.Add(N);
    FindRecursive(N);
  end;
...
begin
  AList.Clear;
  Result := false;
  if not assigned(AVertex) then exit;
  ...
  begin
    ...
    for i := 0 to Triangles.Count - 1 do
      if Triangles[i].VertexIndex(AVertex) >= 0 then begin
        TriBase := Triangles[i];
        break;
      end;
    if not assigned(TriBase) then exit;
  end;
  Result := True;
  AList.Add(TriBase);
...
begin
  ...
  FSegmentChain := TsdTriangleChain2D.Create;
end;
  ...
  begin
    ...
    for i := 0 to 2 do begin
      ...
      if assigned(N) then begin
        if N.RegionIndex >= 0 then continue;
...
begin
  ...
  try
    ...
    repeat
      ...
      if Borders.Count > 0 then begin
        T := Borders[0];
        for i := 0 to 2 do begin
          N := T.Neighbours[i];
          S := T.Segments[i];
          if assigned(N) and assigned(S) then begin
            if N.RegionIndex >= 0 then begin
...
begin
  if assigned(FOnExecutionStep) then FOnExecutionStep(Self, AMessage);
end;

procedure TsdTriangulationMesh2D.DoPhaseComplete(const AMessage: string);
begin
  if assigned(FOnPhaseComplete) then FOnPhaseComplete(Self, AMessage);
end;
...
begin
  ...
  // no triangles?
  if not assigned(ATriangle) then exit;
  ...
  repeat
    ...
    if Result in [httClose0..httClose2] then begin
      ...
      if assigned(Neighbour) then begin
        ATriangle := Neighbour;
        Result := ATriangle.HitTest(APoint);
        inc(FHitTests);
      end;
    ...
    if not assigned(ATriangle) then break;
...
begin
  ...
  if not BoundingBox(FBBMin, FBBMax) then exit;
  ...
  FCornerPoints.Clear;
  FCornerPoints.Add(TsdVertex2D.CreateWithCoords(FMeshMin.X, FMeshMin.Y));
  FCornerPoints.Add(TsdVertex2D.CreateWithCoords(FMeshMax.X, FMeshMin.Y));
  FCornerPoints.Add(TsdVertex2D.CreateWithCoords(FMeshMax.X, FMeshMax.Y));
  FCornerPoints.Add(TsdVertex2D.CreateWithCoords(FMeshMin.X, FMeshMax.Y));
  Tri1 := NewTriangle;
  Tri2 := NewTriangle;
...
procedure TsdTriangulationMesh2D.ReduceSegmentChain(AChain: TsdTriangleChain2D; ARemovals: TsdTriangle2DList);
begin
// default does nothing
end;
...
begin
  ...
  if ARemovalStyle = rsNone then exit;

  for i := Triangles.Count - 1 downto 0 do begin
    T := Triangles[i];
    if T.RegionIndex < 0 then continue;
...
begin
  ...
  for i := 0 to 2 do begin
    ...
    if assigned(V) and (V.Triangle = ATriangle) then begin
      // Point the vertex to one of the neighbours that also shares this triangle
      if assigned(ATriangle.Neighbours[i]) then begin
        V.Triangle := ATriangle.Neighbours[i];
        continue;
      end;
      if assigned(ATriangle.Neighbours[i + 2]) then begin
...
procedure TsdTriangulationMesh2D.ReplaceVertexInSegments(OldVertex, NewVertex: TsdVertex2D);
var
  i, Idx: integer;
begin
...
begin
  ...
  if assigned(N1) then N1.ReplaceNeighbour(Tri0, Tri1);
  if assigned(N2) then N2.ReplaceNeighbour(Tri0, Tri2);
  ...
  // Add to updates
  if assigned(Updates) then begin
    Updates.Add(Tri0);
    Updates.Add(Tri1);
    Updates.Add(Tri2);
  end;
...
begin
  ...
  if assigned(Tri21) then begin
    ...
    if assigned(N1) then N1.ReplaceNeighbour(Tri11, Tri12);
    N2 := Tri21.Neighbours[E2 + 1];
    if assigned(N2) then N2.ReplaceNeighbour(Tri21, Tri22);
  ...
  if assigned(S) then  begin
    ...
    NewS := NewSegment;
    if S.Vertex1 = Tri11.Vertices[E1] then begin
...
begin
  ...
  while i < Segments.Count do
    if AddSegmentToTriangulation(Segments[i]) then
      inc(i);
  DoPhaseComplete('Segment addition');

  // Remove the elements we added for construction
  if ARemovalStyle = rsNone then begin
    ...
    for i := 0 to Triangles.Count - 1 do begin
      ...
      for j := 0 to 2 do begin
        if T.Neighbours[j] = nil then begin
          S := NewSegment;
  ...
  if not (ARemovalStyle in [rsNone, rsOutside]) then begin
