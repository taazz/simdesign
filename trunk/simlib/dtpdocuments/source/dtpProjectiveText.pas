...
uses
  ...
  Classes, dtpShape, dtpPolygonText, dtpGraphics, Math, RelaxXml, dtpHandles, dtpTextShape;
...
procedure TdtpProjectiveText.LoadFromXml(ANode: TXmlNode);
var
  i: integer;
  AList: TList;
begin
  ...
  try
    ...
    begin
      FPoints[i].X := TXmlNode(AList[i]).ReadFloat('X');
      FPoints[i].Y := TXmlNode(AList[i]).ReadFloat('Y');
    end;
  finally
    AList.Free;
  end;
...
procedure TdtpProjectiveText.SaveToXml(ANode: TXmlNode);
var
  i: integer;
begin
  // Save the 4 points
