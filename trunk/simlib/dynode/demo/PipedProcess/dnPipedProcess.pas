...
{    bSuccess := ReadFile(g_hChildStd_OUT_Rd, chBuf, BUFSIZE, dwRead, nil);
    ...
    dwRead := 0;
    bSuccess := false;//ReadFileEx(g_hOutputFile, @chBuf, BUFSIZE, @piOverlap, @CompletionRoutine);
    if (not bSuccess) then
      break;
...
{    bSuccess := ReadFile(g_hChildStd_OUT_Rd, chBuf, BUFSIZE, dwRead, nil);
    ...
    dwRead := 0;
    bSuccess := false;//ReadFileEx(g_hChildStd_OUT_Rd, @chBuf, BUFSIZE, @piOverlap, @CompletionRoutine);
    if (not bSuccess) then
      break;
