...
uses
  Classes, VirtualTrees, pgScene, pgDocument, pgViewPort, Pyro;
...
begin
  ...
  if not assigned(Info) then
    exit;
  Id := Info.ElementId;
  Result := FScene.ElementById(Id);
end;

function TpgSyncTree.GetElementName(AElement: TpgElement): widestring;
begin
  if AElement.PropById(piName).ExistsLocal then
...
procedure TpgSyncTree.SceneChange(Sender: TObject; AElementId,
