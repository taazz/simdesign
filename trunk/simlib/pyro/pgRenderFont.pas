...
uses
  pgFontGDITrueType, pgCommandPath, pgRenderPath;
...
begin
  ...
  for i := 0 to FontCount - 1 do
    if (Fonts[i] is TpgTrueTypeFont) and (TpgTrueTypeFont(Fonts[i]).FontHandle = Handle) then begin
      Result := Fonts[i];
      exit;
    end;
