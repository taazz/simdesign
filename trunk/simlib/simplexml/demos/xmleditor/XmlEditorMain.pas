...
type
  TfrmMain = class(TForm)
    ...
    tsFormattedSource: TTabSheet;
    smFormattedSource: TSynMemo;
    acShowRawSource: TAction;
    acShowFormattedSource: TAction;
...
begin
  ...
  begin
    ...
    try
      ...
      sbMain.SimpleText := sbMain.SimpleText +
        Format('unique strings: %d', [{FXml.SymbolTable.SymbolCount}0]);  //todo
    except
...
begin
  case pcTree.ActivePageIndex of
  0:; // tsXmlTree
  1{tsXmlSource}:       acShowRawSourceExecute(Sender);
  2{tsFormattedSource}: acShowFormattedSourceExecute(Sender)
  end;
end;
...
procedure TfrmMain.acShowRawSourceExecute(Sender: TObject);
begin
  // Show our Xml Document
  FXml.IndentString := '  ';
  if not Assigned(FRawXmlStream) then
    smXmlSource.Text := ''
  else
   smXMLSource.Text := FRawXmlStream.DataString;
end;

procedure TfrmMain.acShowFormattedSourceExecute(Sender: TObject);
begin
  if FXml.IsEmpty or not Assigned(FRawXmlStream) then
      smFormattedSource.Text := ''
    else
      smFormattedSource.Text := FXml.WriteToLocalUnicodeString;
end;
